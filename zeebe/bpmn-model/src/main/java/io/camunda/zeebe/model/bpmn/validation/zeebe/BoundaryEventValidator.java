...
import static io.camunda.zeebe.model.bpmn.util.ModelUtil.validateExecutionListenersDefinitionForEventElement;
...
public class BoundaryEventValidator implements ModelElementValidator<BoundaryEvent> {
  ...
  public void validate(
    ...
    validateExecutionListenersDefinitionForEventElement(
        element,
                  ...
                  eventDefinition -> {
                    if (eventDefinition instanceof CompensateEventDefinition) {
                      ...
                      if (startExecutionListenersDefined) {
                        validationResultCollector.addError(
                            0,
                            "Execution listeners of type 'start' are not supported by [timer, message, error, signal, escalation] boundary events");
                      }
                    }
                  });
        });
